function generatePDFDataFile(pdfDocumentName) {
    let doc = "";
...
function scrollToBottom(remainingHeightToScroll, scrollToLocation, scrollDistance, callback) {
    setTimeout(function () {
        ...
        if (remainingHeightToScroll >= chosenElement.clientHeight) {
            scrollToBottom(remainingHeightToScroll, scrollToLocation, scrollDistance, callback)
        } else {
            setTimeout(function () {
                callback();
            }, 1500)
...
function startPDFGenerate(documentName = "Document") {
    ...
    if (chosenElement.scrollHeight > chosenElement.clientHeight) {
        ...
        scrollToBottom(0, 0, scrollDistance, generatePDF_DataFile);
